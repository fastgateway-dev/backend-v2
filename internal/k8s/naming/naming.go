...
// HTTPRoute returns the name for a base HTTPRoute
func HTTPRoute(baseName string) string {
	return baseName
}

// PerClientHTTPRoute returns the name for a per-client HTTPRoute
// Format: {baseName}-ak-{clientPrefix}
func PerClientHTTPRoute(baseName, clientID string) string {
	clientPrefix := ClientPrefix(clientID)
	return fmt.Sprintf("%s%s%s", baseName, PerClientSuffix, clientPrefix)
}

// SecurityPolicy returns the name for a base SecurityPolicy
// Format: {routeName}-security
func SecurityPolicy(routeName string) string {
	return fmt.Sprintf("%s%s", routeName, SecurityPolicySuffix)
}

// PerClientSecurityPolicy returns the name for a per-client SecurityPolicy
// Format: {routeName}-ak-{clientPrefix}-security
func PerClientSecurityPolicy(routeName, clientID string) string {
	clientPrefix := ClientPrefix(clientID)
	return fmt.Sprintf("%s%s%s%s", routeName, PerClientSuffix, clientPrefix, SecurityPolicySuffix)
}

// BackendTrafficPolicy returns the name for a base BackendTrafficPolicy
// Format: {routeName}-btp
func BackendTrafficPolicy(routeName string) string {
	return fmt.Sprintf("%s%s", routeName, BackendTrafficPolicySuffix)
}

// PerClientBackendTrafficPolicy returns the name for a per-client BackendTrafficPolicy
// Format: {routeName}-ak-{clientPrefix}-btp
func PerClientBackendTrafficPolicy(routeName, clientID string) string {
	clientPrefix := ClientPrefix(clientID)
	return fmt.Sprintf("%s%s%s%s", routeName, PerClientSuffix, clientPrefix, BackendTrafficPolicySuffix)
}

// APIKeySecret returns the name for an API key secret
// Format: fastgateway-apikey-{routeID}-{clientPrefix}
func APIKeySecret(routeID, clientID string) string {
	clientPrefix := ClientPrefix(clientID)
	return fmt.Sprintf("%s%s-%s", APIKeySecretPrefix, routeID, clientPrefix)
}

// APIKeySecretForClient returns the name for a client's API key secret (stored in central namespace)
// Format: fastgateway-apikey-{clientPrefix}
func APIKeySecretForClient(clientID string) string {
	clientPrefix := ClientPrefix(clientID)
	return fmt.Sprintf("%s%s", APIKeySecretPrefix, clientPrefix)
}

// ClientPrefix returns the shortened prefix for a client ID
