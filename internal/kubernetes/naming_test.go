...
func TestHTTPRoute(t *testing.T) {
	assert.Equal(t, "my-route", HTTPRouteName("my-route"))
	assert.Equal(t, "", HTTPRouteName(""))
}
...
func TestPerClientHTTPRoute(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := PerClientHTTPRouteName(tt.baseName, tt.clientID)
...
func TestSecurityPolicy(t *testing.T) {
	assert.Equal(t, "my-route-security", SecurityPolicyName("my-route"))
	assert.Equal(t, "-security", SecurityPolicyName(""))
}

func TestPerClientSecurityPolicy(t *testing.T) {
	result := PerClientSecurityPolicyName("my-route", "client-1234567890")
	assert.Equal(t, "my-route-ak-client-1-security", result)

	result2 := PerClientSecurityPolicyName("my-route", "short")
	assert.Equal(t, "my-route-ak-short-security", result2)
}

func TestBackendTrafficPolicy(t *testing.T) {
	assert.Equal(t, "my-route-btp", BackendTrafficPolicyName("my-route"))
	assert.Equal(t, "-btp", BackendTrafficPolicyName(""))
}

func TestPerClientBackendTrafficPolicy(t *testing.T) {
	result := PerClientBackendTrafficPolicyName("my-route", "client-1234567890")
	assert.Equal(t, "my-route-ak-client-1-btp", result)

	result2 := PerClientBackendTrafficPolicyName("my-route", "short")
	assert.Equal(t, "my-route-ak-short-btp", result2)
}

func TestEnvoyExtensionPolicy(t *testing.T) {
	assert.Equal(t, "my-route-eep", EnvoyExtensionPolicyName("my-route"))
	assert.Equal(t, "-eep", EnvoyExtensionPolicyName(""))
}

func TestEnvoyExtensionPolicyPerClient(t *testing.T) {
	// EnvoyExtensionPolicyName also covers the per-client case when the
	// caller passes in a name already produced by PerClientHTTPRouteName.
	perClientRoute := PerClientHTTPRouteName("my-route", "client-1234567890")
	assert.Equal(t, "my-route-ak-client-1-eep", EnvoyExtensionPolicyName(perClientRoute))
}

func TestHTTPRouteFilter(t *testing.T) {
	assert.Equal(t, "my-route-hrf", HTTPRouteFilterName("my-route"))
	assert.Equal(t, "-hrf", HTTPRouteFilterName(""))
}

func TestAPIKeySecret(t *testing.T) {
	result := APIKeySecretName("route-abc", "client-1234567890")
	assert.Equal(t, "fastgateway-apikey-route-abc-client-1", result)

	result2 := APIKeySecretName("route-abc", "cl")
	assert.Equal(t, "fastgateway-apikey-route-abc-cl", result2)
}

func TestAPIKeySecretForClient(t *testing.T) {
	result := APIKeySecretForClientName("client-1234567890")
	assert.Equal(t, "fastgateway-apikey-client-1", result)

	result2 := APIKeySecretForClientName("short")
...
func TestClientPrefix(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ClientPrefixName(tt.clientID)
			assert.Equal(t, tt.expected, result)
