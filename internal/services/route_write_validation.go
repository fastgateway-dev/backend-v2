...
func (s *RouteService) validateRouteTrafficPolicies(config *models.RouteConfig, btp *routeplan.BackendTrafficPolicyInput, projectID uuid.UUID) error {
	// Validate backend required fields (namespace, service, port)
	if err := s.validateBackendRequiredFields(config); err != nil {
		return err
	}

	// Validate backend namespaces are managed by the project
	if err := s.validateBackendNamespaces(projectID, config); err != nil {
		return err
	}

	// Validate mirror targets (must be different from primary backends)
	if err := s.validateMirrorTargets(config); err != nil {
		return err
	}

	// Validate failover configuration (must have at least one primary when fallback exists)
	if err := s.validateFailoverConfig(config); err != nil {
		return err
	}
...
func (s *RouteService) validateRouteShapeAndConflicts(config *models.RouteConfig, btp *routeplan.BackendTrafficPolicyInput, protocol models.RouteProtocol, domainID uuid.UUID, excludeRouteID *uuid.UUID) error {
	...
	if err := s.validateMatcherConflict(domainID, config, excludeRouteID); err != nil {
		return err
	}
