...
// TestGRPCServiceExactMatch ports
// grpc_route_matching/test_service_exact.py: grpcService Exact match on
// echo.EchoService routes traffic correctly.
func TestGRPCServiceExactMatch(t *testing.T) {
	t.Parallel()
	...
	const wantBody = "hello-service-exact"
	...
	if err != nil {
		t.Fatalf("service exact match: route never became live: %v", err)
	}
	if res.Code != codes.OK {
		t.Fatalf("service exact match: got code %v, want %v", res.Code, codes.OK)
	}

	res, resp, err := echoCall(ctx, wantBody, callOpt)
	if err != nil {
		t.Fatalf("service exact match: request: %v", err)
	}
	if res.Code != codes.OK {
		t.Fatalf("service exact match: got code %v, want %v", res.Code, codes.OK)
	}
	if resp.Body != wantBody {
		t.Fatalf("service exact match: got echoed body %q, want %q", resp.Body, wantBody)
	}
}
